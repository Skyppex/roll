...
impl Mode for Option<cli::Mode> {
    fn eval(
        ...
        let roller = get_roller(cli);

        let mut results = roller(rolls, side_values)?;
                ...
                Modifier::Reroll { amount, condition } => {
                    ...
                    for result in results.iter_mut() {
                        for _ in 0..value {
                            ...
                            let new_roll = roller(1, side_values)?;
...
type Roller = Box<dyn Fn(i64, &[i64]) -> Result<Vec<DiceRolls>, DynError>>;

fn get_roller(cli: &Cli) -> Roller {
    match cli.mode {
        None | Some(cli::Mode::Rng) => Box::new(|rolls: i64, side_values: &[i64]| {
        ...
        Some(cli::Mode::Avg) => Box::new(|rolls: i64, side_values: &[i64]| {
            let mut results = vec![];
        ...
        Some(cli::Mode::Min) => Box::new(|rolls: i64, side_values: &[i64]| {
            let mut results = vec![];
        ...
        Some(cli::Mode::Max) => Box::new(|rolls: i64, side_values: &[i64]| {
            let mut results = vec![];
        ...
        Some(cli::Mode::Med) => Box::new(|rolls: i64, side_values: &[i64]| {
            let mut results = vec![];
        ...
        Some(cli::Mode::Simavg(v)) => Box::new(move |rolls: i64, side_values: &[i64]| {
            let mut evals = vec![];
